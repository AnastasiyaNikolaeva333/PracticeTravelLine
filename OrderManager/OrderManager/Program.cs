class OrderManager
{
    ...
    public static void Main()
    {
        ...
        const int numberDaysDelivery = 3;
        DateTime dateReceipt = DateTime.Today.AddDays(numberDaysDelivery);
        Console.WriteLine($"\n{order.Name}! Ваш заказ {order.Product} в количестве {order.Count} " +
            $"оформлен! Ожидайте доставку по адресу {order.Address} к {dateReceipt}");
    }
